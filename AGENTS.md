...
Runtime terrain landform note: Stage1 terrain variation is expanded from five high-density concept source panels under `art/generated/stage1-concept-panels/`. `npm.cmd run art:stage1-landforms` must generate the five runtime terrain plates, `src/assets/runtime/stage1-landforms-spritesheet.png`, and placement/collider data in `src/data/stage1Landforms.json` from those image sources. Normal play should render the painted terrain plates as the visible stage; the landform sheet is an authoring/debug reference for collider review via `?debug=landforms`, not a semi-transparent patch layer. Do not reintroduce small high-saturation prop scatter or procedural box-like slabs as the primary terrain dressing. Generate the background terrain images first, then attach collision rectangles through `landformColliders` so the playable route reads as connected alley, rooftop, cliff, and gate architecture rather than decorative objects laid over platforms.

Runtime animation note: Player runtime animation now uses idle 12, run 16, small jump 8, big jump rise 10, speed flip jump 16, apex 4, fall 6, wall slide 8, wall kick 8, ground slash 16, air slash 12, hurt 6, and checkpoint respawn 12 frames. Slash runtime animation still uses separate ground 8 and air 6 frame sequences. The implementation is documented in `ANIMATION_EXPANSION_GATE.md` and `art/final-v3-animation/animation-expansion-manifest.json`.
