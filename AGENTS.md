...
Runtime animation note: Stage1 character, slash, telegraph, and Lantern Warden animation uses derived, fixed-cell sprite sheets under `src/assets/runtime/`. They are cut from frozen approved art by `npm.cmd run art:runtime-sheets` so each runtime frame contains one actor/effect only. Do not point gameplay directly back at the generated contact-sheet layouts in `player-spritesheet`, `enemy-spritesheet`, `kite-wraith-preview`, `slash-flipbook`, `telegraph-flipbook`, or `lantern-warden-spritesheet`, because those sheets can expose adjacent poses and make actors/effects appear split or half-cut.
