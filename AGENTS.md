...
Runtime terrain prop note: Stage1 terrain variation is expanded with `src/assets/runtime/stage1-props-spritesheet.png` plus `src/data/stage1VisualProps.json`. The playable Stage1 scene must render 100+ prop sprite instances across all five sections, using varied frames, sizes, alpha, flip, and rotation so the route reads as connected alley/rooftop space rather than five flat plates. Rebuild these derived props with `npm.cmd run art:stage1-props` when route geometry or approved runtime terrain sources change.
