...
Runtime revision note: Stage1 is now a compact one-way vertical slice based on the approved linear structure concept: rain-lantern slash tutorial, short sign-shaft climb, rooftop hazard lesson, one cyan updraft through a Neon Thorn climb, and a small Lantern Warden arena leading directly to the Moon Gate. Damage knockback distance/control lock is reduced to half of the prior tuning. Speed-flip spinning slash no longer uses a Phaser primitive ring; it uses derived flame-ring frames appended to `src/assets/runtime/slash-runtime-spritesheet.png`.
