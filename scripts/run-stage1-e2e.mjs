...
if (shouldRun('mobile-controls')) await record('mobile-controls', () =>
  withPage({ width: 390, height: 844 }, async (page) => {
    ...
    assert(simultaneousAfter.x >= simultaneousBefore.x + 5, `right+jump touch did not preserve horizontal movement: before ${simultaneousBefore.x}, after ${simultaneousAfter.x}`);
