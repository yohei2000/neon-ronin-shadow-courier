...
| Transparent background | Local cutout implemented | Native route does not expose a true transparent-background argument. Runtime player, enemy, and VFX assets are locally alpha-cut from generated opaque sheets and validated for transparent corners / low opaque-paper ratio. |
...
- Some brush contact shadows and generated sheet-layout artifacts may remain, but visible white paper-card backgrounds are removed from runtime player/enemy/VFX assets and guarded by `art:validate-assets`.

## Blockers

- None for producing a Gate B v2 image-generated review package.
- Native true image-editing and native transparency remain unavailable; the accepted fallback for this goal is real image generation plus local extraction/processing, with alpha-cutout validation documented for human review.
