...
export class PreloadScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    this.load.spritesheet(RuntimeEnvironmentAssetKey.Stage1Landforms, RuntimeEnvironmentImageAssets[RuntimeEnvironmentAssetKey.Stage1Landforms], {
      frameWidth: 768,
      frameHeight: 384
    });
    ...
    for (const [key, url] of Object.entries(RuntimeEnvironmentImageAssets)) {
      if (key === RuntimeEnvironmentAssetKey.ItemIcons || key === RuntimeEnvironmentAssetKey.TouchControls || key === RuntimeEnvironmentAssetKey.Stage1Landforms) continue;
  ...
  create(): void {
    ...
    this.scene.start(scene === 'artlab' ? SceneKey.ArtLab : scene === 'stage2' ? SceneKey.Stage2 : scene === 'stage1' ? SceneKey.Stage1 : SceneKey.Title);
  }
